...
public class MainController extends HttpServlet {
    ...
    private String viewDoctors(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String url = "BacSi.jsp";
        try {
            BacSiDAO bsDAO = new BacSiDAO();
            List<BacSiDTO> list = bsDAO.readAll();
            request.setAttribute("bacSiList", list);
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("error", "Có lỗi xảy ra: " + e.getMessage());
        }
    ...
    private String addDoctor(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        return "AddBacSi.jsp";
    }

    private String createDoctor(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String url = "BacSi.jsp";
        try {
            // Get parameters from the form
            ...
            newDoctor.setHoTen(hoTen);
            newDoctor.setSDT(sdt);
            newDoctor.setEmail(email);

            // Save to database
            BacSiDAO bsDAO = new BacSiDAO();
            boolean success = bsDAO.create(newDoctor);

            if (success) {
                request.setAttribute("message", "Thêm bác sĩ thành công!");
                return viewDoctors(request, response);
            } else {
                request.setAttribute("error", "Có lỗi xảy ra khi thêm bác sĩ!");
                return "AddBacSi.jsp";
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private String editDoctor(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String url = "AddBacSi.jsp";
        try {
            String id = request.getParameter("id");
            BacSiDAO bsDAO = new BacSiDAO();
            BacSiDTO doctor = bsDAO.readById(id);
            if (doctor != null) {
    ...
    private String updateDoctor(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            // Get parameters from the form
            int maBS = Integer.parseInt(request.getParameter("maBS"));
            ...
            // Create BacSiDTO with updated info
            BacSiDTO doctor = new BacSiDTO(maBS, hoTen, sdt, email);

            // Update in database
            BacSiDAO bsDAO = new BacSiDAO();
            boolean success = bsDAO.update(doctor);

            if (success) {
                request.setAttribute("message", "Cập nhật thông tin bác sĩ thành công!");
                return viewDoctors(request, response);
            } else {
                request.setAttribute("error", "Có lỗi xảy ra khi cập nhật thông tin!");
                request.setAttribute("doctor", doctor);
    ...
    private String deleteDoctor(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            String id = request.getParameter("id");
            BacSiDAO bsDAO = new BacSiDAO();
            boolean success = bsDAO.delete(id);

            if (success) {
                request.setAttribute("message", "Xóa bác sĩ thành công!");
            } else {
                request.setAttribute("error", "Có lỗi xảy ra khi xóa bác sĩ!");
            }
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("error", "Có lỗi xảy ra: " + e.getMessage());
    ...
    private String updateServices(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            // Get parameters from the form
            ...
            // Create DichVuDTO with updated info
            DichVuDTO dichVu = new DichVuDTO(maDV, tenDV, moTa, gia);

            // Update in database
            DichVuDAO dvDAO = new DichVuDAO();
            boolean success = dvDAO.update(dichVu);

            if (success) {
                request.setAttribute("message", "Cập nhật dịch vụ thành công!");
                return viewServices(request, response);
            } else {
                request.setAttribute("error", "Có lỗi xảy ra khi cập nhật dịch vụ!");
                request.setAttribute("dichVu", dichVu);
                return "AddDichVu.jsp";
            }
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("error", "Có lỗi xảy ra: " + e.getMessage());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (action == null) {
                ...
                } else if (action.equals("addPatient")) { //them benh nhan
                    url = addPatient(request, response);
                } else if (action.equals("createPatient")) { // lay id benh nhan
                    url = createPatient(request, response);
                } else if (action.equals("editPatient")) { //sua tt bn
                ...
                } else if (action.equals("updateServices")) { //cap nhat dich vu
                    url = updateServices(request, response);
                } else if (action.equals("deleteServices")) { //xoa dich vu
                    url = deleteServices(request, response);
                } else if (action.equals("addNewAccountBS")) { //tao tk bs
                ...
                } else if (action.equals("viewDoctors")) { //xem danh sach bac si
                    url = viewDoctors(request, response);
                } else if (action.equals("addDoctor")) { //dan jsp them bac si
                    url = addDoctor(request, response);
                } else if (action.equals("createDoctor")) { //them bac si
                    url = createDoctor(request, response);
                } else if (action.equals("editDoctor")) { //lay id bac si
                    url = editDoctor(request, response);
                } else if (action.equals("updateDoctor")) { //sua bac si
                    url = updateDoctor(request, response);
                } else if (action.equals("deleteDoctor")) { //xoa bac si
                    url = deleteDoctor(request, response);
                }
            }
