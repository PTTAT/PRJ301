...
public class MainController extends HttpServlet {
    ...
    private String AnsQuestion(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!AuthUtils.isLoggedIn(session)) {
            request.setAttribute("error_message", "Please login to access this page.");
            return url;
        }
        ...
        try {
            ...
            if (examIdStr == null || examIdStr.trim().isEmpty()) {
                request.setAttribute("error_message", "Invalid exam ID.");
                return "search.jsp";
            }

            int examId = Integer.parseInt(examIdStr);
            AnsQuestionDAO ansQuestionDAO = new AnsQuestionDAO();
            List<AnsQuestionDTO> questions = ansQuestionDAO.getQuestionsByExamId(examId);
            
            if (questions == null || questions.isEmpty()) {
                request.setAttribute("error_message", "No questions found for this exam.");
                return "search.jsp";
            }
            ...
            request.setAttribute("error_message", "Invalid exam ID format.");
            url = "search.jsp";
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("error_message", "An error occurred while loading questions.");
            url = "search.jsp";
        }
        return url;
    }
