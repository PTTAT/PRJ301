...
public class MainController extends HttpServlet {
    ...
    private String AnsQuestion(HttpServletRequest request, HttpServletResponse response) {
        String url = LOGIN_PAGE;
        HttpSession session = request.getSession();
        ...
        if (AuthUtils.isUser(session)) {
            try {
                String examIdStr = request.getParameter("exam_id");
                System.out.println("Received exam_id parameter: " + examIdStr);
                
                if (examIdStr != null && !examIdStr.trim().isEmpty()) {
                    int examId = Integer.parseInt(examIdStr);
                    System.out.println("Parsed exam_id: " + examId);
                    
                    AnsQuestionDAO questionDAO = new AnsQuestionDAO();
                    List<AnsQuestionDTO> questions = questionDAO.getQuestionsByExamId(examId);
                    
                    System.out.println("Retrieved questions count: " + (questions != null ? questions.size() : "null"));
                    request.setAttribute("ques", questions);
                    url = "ansQuestion.jsp";
                } else {
                    System.out.println("exam_id parameter is null or empty");
                }
            } catch (Exception e) {
                System.out.println("Error in AnsQuestion: " + e.toString());
                e.printStackTrace();
            }
        } else {
    ...
    private String submitAnswers(HttpServletRequest request, HttpServletResponse response) {
        String url = LOGIN_PAGE;
        HttpSession session = request.getSession();
        ...
        if (AuthUtils.isUser(session)) {
            try {
                ...
                if (examIdStr != null && !examIdStr.trim().isEmpty()) {
                    ...
                    int score = 0;
                    // Check each answer
                    for (AnsQuestionDTO question : questions) {
                        String userAnswer = request.getParameter("answer_" + question.getQuestion_id());
                    ...
                    request.setAttribute("ques", questions); // Keep the questions in the request to display them again
                    
                    url = "ansQuestion.jsp";
                }
            } catch (Exception e) {
                System.out.println("Error in submitAnswers: " + e.toString());
                e.printStackTrace();
            }
        }
        return url;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (action == null) {
                ...
                } else if (action.equals("ansQuestion")) {
                    url = AnsQuestion(request, response);
                } else if (action.equals("submitAnswers")) {
                    url = submitAnswers(request, response);
                }
            }
